...
# HELPERS
    ...
    def collection_by_prop(team_hash, prop)
        team_hash[:players].collect{ |player_name, stats| stats[prop]}
    end

    # Max by stat

    def max_player_by_stat(stat_name)
        merged_players.max_by{|player_name, stats| stats[stat_name]}[0]
    end

    # Max by stat

    def stats_by_max_value(stat_name)
        merged_players.max_by{|player_name, stats| stats[stat_name]}[1]
    end
...
# Methods Required - I will use [] notation over fetch
    ...
    def shoe_size(player_name)
        merged_players[player_name][:shoe]
    end
...
# Bonus
    ...
    def player_with_longest_name
        merged_players.max_by{|player_name, stats| player_name.size }[0]
    end
